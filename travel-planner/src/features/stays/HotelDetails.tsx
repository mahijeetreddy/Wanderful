...
function PropertyPhoto({ url, label }: { url: string; label: string }) {
  const [failed, setFailed] = useState(false);
  return failed ? <div role="img" aria-label={`${label}: unavailable`} className="grid h-36 place-items-center rounded-2xl border border-white/20 bg-[#17333a] px-3 text-center text-sm text-[#c3e5e5]">Photo unavailable</div> : <img src={url} alt={label} loading="lazy" onError={() => setFailed(true)} className="h-36 w-full rounded-2xl object-cover" />;
}
