...
export function TripOverview({ form, options, itinerary, onNavigate }: { form: PlannerForm; options: PlannerOptions; itinerary: StructuredItineraryData | null; onNavigate: (tab: ResultTab) => void }) {
  ...
  const conflicts = (itinerary?.days || []).flatMap(day => (day.activities || []).filter(activity => activity.schedule_conflict));
  return <section aria-label="Trip overview" className="space-y-5 p-4 sm:p-6">
    {!!conflicts.length && <div role="status" className="rounded-2xl border border-amber-200/30 bg-amber-200/10 p-4 text-amber-100"><p>{conflicts.length} activities need replanning after your flight change.</p><p className="mt-2 text-sm">{conflicts.map(activity => activity.title).join(" · ")}</p><button onClick={() => onNavigate("itinerary")} className="mt-2 min-h-11 underline">Review itinerary</button></div>}
