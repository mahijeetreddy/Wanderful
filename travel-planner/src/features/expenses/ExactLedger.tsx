...
export function ExactLedger({ trip, onUpdated }: { trip: SavedTrip; onUpdated: (trip: SavedTrip) => void }) {
  ...
  const [form, setForm] = useState({ label: "", amount: "", paid_by_id: "", split_ids: [] as string[], commitment_id: "", category: "Food" });
  ...
  const splits = form.split_ids.length ? form.split_ids : members.map(member => member.id);
