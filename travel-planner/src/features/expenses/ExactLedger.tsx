...
export function ExactLedger({ trip, onUpdated }: { trip: SavedTrip; onUpdated: (trip: SavedTrip) => void }) {
  ...
  return <section aria-label="Trip money ledger" className="mt-6 space-y-5 text-white">
    ...
    {ledger.reserve_minor !== undefined && <BudgetReserve trip={trip} revision={ledger.revision} percent={ledger.reserve_percent || "0"} amount={money(ledger.reserve_minor)} remaining={money(ledger.spendable_remaining_minor || 0)} onUpdated={onUpdated} />}
